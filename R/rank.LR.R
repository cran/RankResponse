...
rank.LR=function(data,response.number,prior.parameter,c)
{
      data=as.matrix(data)
      v=response.number
	...
	Loss = numeric(v-1)
	Loss[1] = sum(u)/((v-1)+epsilon)

	for(i in 2:(v-1)){
		Loss[i] = c*(i-1)/(i-1+epsilon) - (c/(i-1+epsilon)+1/((v-1)-(i-1)+epsilon))*sum(V_star[((v-1)-(i-1)+1):(v-1)]) + (1/((v-1)-(i-1)+epsilon))*sum(u)
	}
	D_star = order(Loss)[1]-1
	t_LR = V_star[(v-1)-D_star]
